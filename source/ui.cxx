...
void ui::update_ui_state() {
  ...
  if (m_state == just_looking && IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) {
    m_state = popup_menu;
    m_popup = std::make_unique<popup>(
        std::vector<std::tuple<std::string_view, popup_actions>>{
  ...
  if (m_state == drawing_new_note) {
    if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) {
      ...
      // TODO: sort notes in a descending order based on their size.
      // So that when rendering, we do not cover small notes with big ones.

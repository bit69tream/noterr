...
namespace ui {
  ...
  : m_box(box), m_border_box(box), m_title(std::move(title)), m_text(std::move(text)), m_theme(theme) {
  ...
  void note::compute_element_coordinates() {
    ...
    Vector2 title_size = MeasureTextEx(m_theme.font, m_title.c_str(), m_theme.font_size, m_theme.font_spacing);
  ...
  void note::render() const {
    ...
    DrawTextEx(m_theme.font, m_title.c_str(), m_title_position, m_theme.font_size, m_theme.font_spacing, m_theme.note_foreground);
    DrawRectangleRec(m_splitter_line, m_theme.border);

    DrawTextEx(m_theme.font, m_text.c_str(), m_text_position, m_theme.font_size, m_theme.font_spacing, m_theme.note_foreground);
  }
};
